...
- Performance benchmark suite restructured around five scenarios. Renamed `tests/performance/backend-performance.test.ts` → `tests/performance/rm-backend-performance.test.ts` and added additional env vars so a single test file covers every rm-connector-js scenario. Removed three redundant benchmark files — their data is now produced from the consolidated test plus the existing native baseline. Moved `bench-runs.sh` into `tests/performance/` and rewrote phases. Output filenames now include a `loopback` / `remote` location label. The native baseline test now skips only the idb side when `idb-pconnector` is unavailable (e.g. when running from a workstation), so the mapepire side can still produce remote baseline data. Pool config: rm-connector-js (`RmPool`) grows on demand from 5 in increments of 5 up to `maxSize: 1000`. Native baseline pre-allocates 50 connections per backend for Pool — Promise.all (raw `Connection` objects round-robin for idb; `Pool` pre-sized at 50 for mapepire) because `DBPool.attach()` serialises pathologically under burst and the native mapepire `Pool` has no growth rate-limiter. Native idb Pool — sequential keeps `DBPool` since contention isn't an issue there. See `docs/PERFORMANCE-COMPARISON.md` for the new scenario layout.
